const colors = {
    main: "rgb(17, 39, 58)", //#11273a
    mainSecondary: "rgb(101, 126, 142)", //#657e8e
    mainTertiary: "rgb(156, 189, 210)", //#9bbcd1
    mainQuartiary: "rgb(177, 194, 205)", //#b1c2cd
    lightMain: "rgb(79, 141, 180)", //#4F8DB4
    lightMainSecondary: "rgb(103, 190, 242)", //#67bef2
    lightMainTertiary: "rgb(218, 232, 234)", //#dae8ea
    black: "rgb(17, 17, 17)", //#111111
    blackSecondary: "rgb(81, 81, 81)", //#515151
    blackTertiary: "rgb(32, 34, 34)", // #202222
    neutral: "rgb(76, 77, 78)", //#4d4e4f
    neutralSecondary: "rgb(151, 154, 155)", //#979a9b
    neutralTertiary: "rgb(194, 200, 204)", //#c2c8cc
    lightNeutral: "rgb(245, 245, 245)", //#f5f5f5
    lightNeutralSecondary: "rgb(238, 238, 238)", //#eeeeee
    white: "rgb(255, 255, 255)", //#ffffff
    warning: "rgba(219, 159, 13, 0.3)", //#db9f0d
    Shadow: "rgba(0, 0, 0, 0.25)", //#00000040
    logo: "#E8E8E8",
    error: "#ff4d4f",
    active: "#49DE80",
    inactive: "#FBCC13",
};
export default colors;
