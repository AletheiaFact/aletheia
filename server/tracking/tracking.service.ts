        ...
        try {
            ...
            if (!history || history.length === 0) {
                throw new NotFoundException(
                    `Verification request history for ID "${verificationRequestId}" not found.`
                );
            }
            ...
            const tracking = history
                .filter(
                    (history) =>
                        history.details?.after?.status &&
                        history.details?.before?.status !==
                            history.details?.after?.status
                )
                .map((history) => ({
                    id: history._id,
            ...
            if (error instanceof NotFoundException) {
                this.logger.warn(
                    `Tracking not found for ID: ${verificationRequestId}`
                );
                throw error;
            }
