...
export class VerificationRequestStateMachine extends StateMachineBase<VerificationRequestStateMachineContext> {
    ...
    constructor({
        ...
        this.stateMachineService = {
            verificationRequestService: getVerificationRequestService(),
        };
    }

    protected async getEntityCurrentState(
        ...
        const verificationRequest =
            await this.getVerificationRequestService().getById(
                context.verificationRequest?.id
            );
        // For PRE_TRIAGE status, determine next state based on what's been executed
        if (
            verificationRequest?.status === VerificationRequestStatus.PRE_TRIAGE
        ) {
            ...
            const expectedStates = [
                {
                    field: "embedding",
                    state: VerificationRequestStateMachineStates.EMBEDDING,
                },
                {
                    field: "identifiedData",
                    state: VerificationRequestStateMachineStates.IDENTIFYING_DATA,
                },
                {
                    field: "topics",
                    state: VerificationRequestStateMachineStates.DEFINING_TOPICS,
                },
                {
                    field: "impactArea",
                    state: VerificationRequestStateMachineStates.DEFINING_IMPACT_AREA,
                },
                {
                    field: "severity",
                    state: VerificationRequestStateMachineStates.DEFINING_SEVERITY,
                },
            ];
        ...
        return (
            statusMap[
                verificationRequest?.status as VerificationRequestStatus
