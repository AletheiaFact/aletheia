...
describe("SourceController (e2e)", () => {
    ...
    it("/api/source/target/:targetId (GET) - Should get sources by targetId", () => {
        return request(app.getHttpServer())
            ...
            .expect(200)
            .expect(({ body }) => {
                const sourceTargetId = Array.isArray(body.sources[0].targetId)
                    ? body.sources[0].targetId[0]
                    : body.sources[0].targetId;
