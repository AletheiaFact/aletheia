...
describe("ReviewTaskService - Notification Functionality", () => {
    ...
    beforeEach(async () => {
        ...
        notificationService = testingModule.get(
            NotificationService
        ) as jest.Mocked<NotificationService>;

        // Mock the i18n utility
        jest.spyOn(i18nUtil, "getTranslation").mockImplementation(
            (locale, key) => {
                const translations = {
    ...
    describe("_generateNotificationUrl()", () => {
        ...
        it("should generate correct claim-based URL for sentence content", async () => {
            ...
            expect(url).toBe(
                "https://aletheiafact.org/test-environment/claim/test-claim-slug/sentence/test-sentence-hash"
            );
        });
        ...
        it("should omit namespace prefix for Main namespace", async () => {
            ...
            expect(url).toBe(
                "https://aletheiafact.org/claim/test-claim-slug/sentence/test-sentence-hash"
            );
        });
        ...
        it("should generate correct URL for source content", async () => {
            ...
            expect(url).toBe(
                "https://aletheiafact.org/test-environment/source/test-source-hash"
            );
        });
        ...
        it("should generate correct URL for verification request", async () => {
            ...
            expect(url).toBe(
                "https://aletheiafact.org/test-environment/verification-request/test-verification-hash"
            );
        });
        ...
        it("should use default BASE_URL when environment variable not set", async () => {
            ...
            expect(url).toBe(
                "https://aletheiafact.org/test-environment/claim/test-slug/sentence/test-hash"
            );
    ...
    describe("_sendNotificationsForStateChange()", () => {
            ...
            expect(notificationService.sendNotification).toHaveBeenCalledWith(
                "user1",
                {
                    messageIdentifier:
                        "You have been assigned to start a review",
                    redirectUrl:
                        "https://aletheiafact.org/test-environment/claim/test-claim-slug/sentence/test-hash",
                }
            );
            expect(notificationService.sendNotification).toHaveBeenCalledWith(
                "user2",
        ...
        it("should not send notifications to current user", async () => {
            ...
            ).not.toHaveBeenCalledWith("current-user-id", expect.any(Object));
        });
