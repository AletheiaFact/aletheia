...
export async function up(db: Db) {
    ...
    try {
        ...
        while (await claimCollectionCursor.hasNext()) {
            ...
            const debate = await db.collection("debates").insertOne({
                isLive: doc.isLive,
                type: "debate",
                data_hash,
            });
	        ...
	        add the personality to the speech and add the speech to the debate,
	        ...
	        */
            ...
            for (let i = 0; i < editorContentObject.content.length; i++) {
                if (
                    editorContentObject.content[i].type === "editor-claim-card"
                ) {
